...
Namespace ScriptBuilder
    ...
    Public Class RCodeStructure
        ...
        Private ReadOnly _dataBook As String = "data_book"
        ...
        Public Overridable Function ToScript(Optional ByRef strScript As String = "", Optional strTemp As String = "") As String
            ...
            If bToBeAssigned Then
                ...
                If Not strAssignToDataFrame = "" AndAlso (Not strAssignToColumn = "" OrElse bAssignToColumnWithoutNames) Then
                    clsAddColumns.SetRCommand(_dataBook & "$add_columns_to_data")
                    clsAddColumns.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
                    'if we need to assign to a named column
                    If Not bAssignToColumnWithoutNames Then
                        clsAddColumns.AddParameter("col_name", ChrW(34) & strAssignToColumn & ChrW(34))
                    End If
                    ...
                    Else
                        'ToDo need to be able to see this option
                    ...
                    clsGetColumns.SetRCommand(_dataBook & "$get_columns_from_data")
                    clsGetColumns.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
                    clsGetColumns.AddParameter("col_names", ChrW(34) & strAssignToColumn & ChrW(34))
                ...
                ElseIf Not strAssignToModel = "" Then 'else if we need to assign to a model
                    clsAddModels.SetRCommand(_dataBook & "$add_model")
                    clsAddModels.AddParameter("model_name", ChrW(34) & strAssignToModel & ChrW(34))
                    clsAddModels.AddParameter("model", strAssignTo)
                    clsAddModels.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
                    If Not strAssignToDataFrame = "" Then
                        clsAddModels.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
                        clsGetModels.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
                    End If
                    strScript = strScript & clsAddModels.ToScript() & Environment.NewLine

                    clsGetModels.SetRCommand(_dataBook & "$get_models")
                    clsGetModels.AddParameter("model_name", ChrW(34) & strAssignToModel & ChrW(34))
                    strAssignTo = clsGetModels.ToScript()
                ElseIf Not strAssignToGraph = "" Then 'else if we need to assign to a graph
                    clsAddGraphs.SetRCommand(_dataBook & "$add_graph")
                    clsAddGraphs.AddParameter("graph_name", ChrW(34) & strAssignToGraph & ChrW(34))
                    clsAddGraphs.AddParameter("graph", strAssignTo)
                    If Not strAssignToDataFrame = "" Then
                        clsAddGraphs.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
                        clsGetGraphs.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
                    End If
                    strScript = strScript & clsAddGraphs.ToScript() & Environment.NewLine

                    clsGetGraphs.SetRCommand(_dataBook & "$get_graphs")
                    clsGetGraphs.AddParameter("graph_name", ChrW(34) & strAssignToGraph & ChrW(34))
                    strAssignTo = clsGetGraphs.ToScript()
                ElseIf Not strAssignToSurv = "" Then 'else if we need to assign to a surv
                    clsAddSurv.SetRCommand(_dataBook & "$add_surv")
                    clsAddSurv.AddParameter("surv_name", ChrW(34) & strAssignToSurv & ChrW(34))
                    clsAddSurv.AddParameter("surv", strAssignTo)
                    If Not strAssignToDataFrame = "" Then
                        clsAddSurv.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
                        clsGetSurv.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
                    End If
                    strScript = strScript & clsAddSurv.ToScript() & Environment.NewLine

                    clsGetSurv.SetRCommand(_dataBook & "$get_surv")
                    clsGetSurv.AddParameter("surv_name", ChrW(34) & strAssignToSurv & ChrW(34))
                    strAssignTo = clsGetSurv.ToScript()

                ElseIf Not strAssignToTable = "" Then 'else if we need to assign to a table
                    clsAddTables.SetRCommand(_dataBook & "$add_table")
                    clsAddTables.AddParameter("table_name", ChrW(34) & strAssignToTable & ChrW(34))
                    clsAddTables.AddParameter("table", strAssignTo)
                    If Not strAssignToDataFrame = "" Then
                        clsAddTables.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
                        clsGetTables.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
                    End If
                    strScript = strScript & clsAddTables.ToScript() & Environment.NewLine

                    clsGetTables.SetRCommand(_dataBook & "$get_tables")
                    clsGetTables.AddParameter("table_name", ChrW(34) & strAssignToTable & ChrW(34))
                    strAssignTo = clsGetTables.ToScript()
                ElseIf Not strAssignToDataFrame = "" Then 'else if we need to assign to a data frame
                    clsAddData.SetRCommand(_dataBook & "$import_data")
                    ...
                    clsGetData.SetRCommand(_dataBook & "$get_data_frame")
                    clsGetData.AddParameter("data_name", ChrW(34) & strAssignToDataFrame & ChrW(34))
        ...
        Private Function ConstructAssignTo(strAssignTo As String, strTemp As String) As String
            ...
            If Not String.IsNullOrEmpty(strTemp) Then
                ...
                arrScriptParts = GetRunnableCommandLines(strTemp)
