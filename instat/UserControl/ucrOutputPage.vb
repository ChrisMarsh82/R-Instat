...
Public Class ucrOutputPage
    ...
    Private _canReOrder As Boolean
    Private _canRename As Boolean
    ...
    ''' </summary>
    ''' <returns></returns>
    Public ReadOnly Property SelectedElements() As List(Of OutputElement)
        Get
            Dim elemets As New List(Of OutputElement)
    ...
    ''' </summary>
    ''' <returns></returns>
    Public Property CanReOrder() As Boolean
        Get
            Return _canReOrder
        End Get
    ...
    Public Sub AddNewOutput(outputElement As OutputElement)
    ...
    Public Sub Save(path As String)
        Dim richText As New RichTextBox
        For Each checkbox In _checkBoxes
            Dim element As OutputElement = checkbox.Tag
            AddElementToRichTextBox(element, richText)
        Next
        richText.SaveFile(path)
    End Sub

    Private Function AddElementPanel(outputElement As OutputElement) As Panel
    ...
    Private Sub AddCheckBoxToElementPanel(panel As Panel, outputElement As OutputElement)
    ...
    Private Sub AddNewScript(outputElement As OutputElement)
    ...
    Private Function CopyOneImageOnly() As Boolean
        If SelectedElements.Count = 1 And SelectedElements(0).OutputType = OutputType.ImageOutput Then
            Dim element As OutputElement = SelectedElements(0)
            Clipboard.Clear()
    ...
    Private Sub AddElementToRichTextBox(element As OutputElement, richText As RichTextBox)
    ...
    Private Sub FillRichTextBoxWithFormatedRScript(richTextBox As RichTextBox, formatedRScript As List(Of RScriptElement))
    ...
    Private Sub AddNewTextOutput(outputElement As OutputElement)
        Dim richTextBox As New RichTextBox With {
           .Dock = DockStyle.Top,
           .BorderStyle = BorderStyle.None
        }
    ...
    Private Sub AddNewImageOutput(outputElement As OutputElement)
